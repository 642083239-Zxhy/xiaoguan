...
const INTENT_RULES = [
  ...
  ['complaint', ['投诉', '欺骗', '差评', '赔偿', '太差', '垃圾', '消协'], 0.97],
  ...
  ['bargain', ['便宜点', '最低价', '最低多少', '打折', '优惠', '别家更便宜'], 0.94],
...
const nextActionFor = (intent, entities) => {
  if (intent === 'security_warning') return '拒绝';
...
export const routeSalesMessage = ({ message, currentCriteria, skus, faqs }) => {
  ...
  if (intent === 'complaint') return { analysis, criteriaUpdates, response: { type: 'text', content: '很抱歉给你带来不好的体验。当前版本不处理人工转接，我已停止推荐；请通过正式售后渠道提交订单信息处理。' } };
