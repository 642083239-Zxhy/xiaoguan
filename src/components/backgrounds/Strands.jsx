...
void main() {
  ...
  float pointerInfluence = exp(-length((uv - pointer) * vec2(1.15, 1.8)) * 4.2);
  uv.y -= uPointer.y * pointerInfluence * 0.09 * uInteractionStrength;
  uv.x += uPointer.x * pointerInfluence * 0.035 * uInteractionStrength;
...
export default function Strands({
  ...
  useEffect(() => {
    ...
    const update = t => {
      ...
      program.uniforms.uPointer.value = current.mouseInteraction
        ? [pointerCurrent.x, pointerCurrent.y]
        : [0, 0];
      program.uniforms.uInteractionStrength.value = current.interactionStrength;
